...
class MemoryRetriever:
    ...
    def retrieve_relevant_memories(
        ...
        for memory in all_memories:
            relevance_score = self._calculate_relevance(
                memory, current_turn, user_message
            )
        ...
        diverse_memories = self._apply_diversity_filter(scored_memories, max_memories)
    ...
    def _calculate_relevance(
        self, memory: Dict[str, Any], current_turn: int, user_message: str
    ) -> float:
        """
        ...
        semantic_score = self._calculate_semantic_similarity(
            memory.get("content", ""), user_message
        )
    ...
    def _apply_diversity_filter(
        ...
        for memory in memories:
            ...
            if type_counts[memory_type] < 2:  # Max 2 per type
                ...
                if (
                    memory_key not in seen_keys
                    or len(diverse_memories) < max_count // 2
                ):
    ...
    def retrieve_by_type(
        self, session_id: str, memory_type: str, max_memories: int = 10
    ) -> List[Dict[str, Any]]:
        """Retrieve memories of a specific type"""
        memories = self.storage.get_session_memories(
            session_id, memory_type=memory_type
        )

        # Sort by confidence and recency (newest first on ties)
        memories.sort(
    ...
    def retrieve_by_turn_range(
        ...
        filtered = [
            m for m in all_memories if start_turn <= m.get("source_turn", 0) <= end_turn
        ]
    ...
    def format_memories_for_prompt(
        self, memories: List[Dict[str, Any]], include_metadata: bool = False
    ) -> str:
        """
        ...
        for mem_type, mems in by_type.items():
            ...
            for mem in mems:
                ...
                if include_metadata:
                    ...
                    formatted.append(
                        f"- {content} (from turn {turn}, confidence: {conf:.2f})"
                    )
                else:
    ...
    def get_retrieval_stats(self, session_id: str) -> Dict[str, Any]:
        ...
        avg_confidence = sum(m.get("confidence", 0) for m in all_memories) / len(
            all_memories
        )
