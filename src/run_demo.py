...
def print_turn(turn_num, user_msg, response):
    ...
    if response.get("extracted_memories"):
        ...
        for mem in response["extracted_memories"]:
            print(
                f"   • {mem['content']} [{mem['type']}] (confidence: {mem['confidence']:.2f})"
            )
...
def run_demo():
    ...
    print_header("LONG-FORM MEMORY SYSTEM - PERSONAL DEMO")
    ...
    conversation = [
        ...
        # Long-range recall checkpoint (Turn 937)
    ...
    for user_msg, turn_num in conversation:
        response = agent.process_turn(
            session_id=session_id, user_message=user_msg, turn_number=turn_num
        )
    ...
    print(
        f"   Memory range: Turn {summary['memory_stats']['earliest_turn']} to {summary['memory_stats']['latest_turn']}"
    )
    ...
    # Performance metrics
    print_header("PERFORMANCE CHECKLIST")
...
if __name__ == "__main__":
    try:
        ...
        print("\nTroubleshooting:")
        print(
            "  1. Make sure dependencies are installed: pip install -r requirements-minimal.txt"
        )
