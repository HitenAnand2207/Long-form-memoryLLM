...
class TestMemoryStorage:
    ...
    def test_embeddings_path_is_relative_to_db_dir(self):
        """Test that embedding artifacts are colocated with the configured DB directory"""
        expected_embeddings_dir = os.path.join(self.temp_dir, "embeddings")
        assert self.storage.embeddings_dir == expected_embeddings_dir
...
class TestMemoryRetrieval:
    ...
    def test_recency_weighting(self):
        ...
        relevant = self.retriever.retrieve_relevant_memories(
            session_id=session_id, current_turn=105, user_message="test", max_memories=3
        )
    ...
    def test_retrieve_by_type_prefers_recent_on_confidence_tie(self):
        """Test that retrieve_by_type returns newer memories first when confidence is equal"""
        session_id = "test_session"
...
class TestConversationAgent:
    ...
    def test_performance_metrics(self):
        ...
        response = self.agent.process_turn(
            session_id="test_session", user_message="Test message", turn_number=1
        )
